...
export function ArchiveView({ all }: { all: EventItem[] }) {
  ...
  const items = localizedEvents(archiveBySeries(publicEventLists(all).archive, series), locale);
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 pt-24 pb-20 md:px-8 md:pt-28 md:pb-28">
      ...
      <h1 className="mt-3 font-serif text-3xl tracking-wide md:text-4xl">{series || t.archive.title}</h1>
      ...
      <div className="mt-10 md:mt-12">
        {items.length === 0 ? (
          ...
          <EventList items={items} programs={localizedEvents(all, locale)} nestedPrograms={false} />
        )}
      </div>
    </div>
  );
